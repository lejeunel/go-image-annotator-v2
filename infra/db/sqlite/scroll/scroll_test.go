...
func TestInternalErrOnGetAdjacent(t *testing.T) {
	repos := NewTestScrollerRepos()
	repos.Scroller.Db.Close()
	_, err := repos.Scroller.GetAdjacent(im.NewImageId(), scroller.NewScrollingCriteria(), scroller.ScrollNext)
	if !errors.Is(err, e.ErrInternal) {
		t.Fatalf("expected internal error, got %v", err)
	}
}

func TestImageMustExistShouldFailWhenImageDoesNotExist(t *testing.T) {
	repos := NewTestScrollerRepos()
	err := repos.Scroller.ImageMustExist(im.NewImageId())
...
func TestCollectionMustExistShouldFailWhenCollectionDoesNotExist(t *testing.T) {
	repos := NewTestScrollerRepos()
...
func TestShouldFailWhenNoImage(t *testing.T) {
	repos := NewTestScrollerRepos()
	id := im.NewImageId()
	_, err := repos.Scroller.GetAdjacent(id, scroller.NewScrollingCriteria(), scroller.ScrollNext)
	if !errors.Is(err, e.ErrNotFound) {
		t.Fatalf("expected not found error got %v", err)
	}
}
...
func TestGettingAdjacentImageWhenSingleImageShouldFail(t *testing.T) {
	...
	_, err := repos.Scroller.GetAdjacent(id, scroller.NewScrollingCriteria(), scroller.ScrollPrevious)
	if !errors.Is(err, e.ErrNotFound) {
...
func TestGettingNextImage(t *testing.T) {
	...
	r, _ := repos.Scroller.GetAdjacent(ids[1], scroller.NewScrollingCriteria(), scroller.ScrollNext)
	if r.ImageId != ids[2] {
...
func TestGettingPrevImage(t *testing.T) {
	repos := NewTestScrollerRepos()
	ids := CreateImagesWithOrderedIds(repos.Image, 3)
	r, _ := repos.Scroller.GetAdjacent(ids[2], scroller.NewScrollingCriteria(), scroller.ScrollPrevious)
	if r.ImageId != ids[1] {
		t.Fatalf("expected to retrieve next image with id %v got %v", ids[1], r.ImageId)
	}
}

func TestGettingAdjacentImageInCollectionWhenSingleImageShouldFail(t *testing.T) {
	repos := NewTestScrollerRepos()
	...
	_, err := repos.Scroller.GetAdjacent(firstImage.Id,
		scroller.NewScrollingCriteria(scroller.WithCollection("first-collection")),
...
func TestGettingNextImageInCollection(t *testing.T) {
	...
	repos.Image.AddImageToCollection(ids[0], collection.Id)
	repos.Image.AddImageToCollection(ids[1], collection.Id)

	r, _ := repos.Scroller.GetAdjacent(ids[0],
		scroller.NewScrollingCriteria(scroller.WithCollection(collection.Name)),
		scroller.ScrollNext)
