...
func TestNonExistingCollectionShouldFail(t *testing.T) {
	...
	itr := NewInteractor(&FakeRepo{MissingCollection: true}, &i.FakeArtefactRepo{}, presenter)
	itr.Execute(Request{})
...
func TestInvalidImageDataShouldFail(t *testing.T) {
	presenter := &FakePresenter{}
	repo := &FakeRepo{}
	itr := NewInteractor(repo, &i.FakeArtefactRepo{}, presenter)
...
func TestHandleInternalErrorOnCollectionExistsCheck(t *testing.T) {
	presenter := &FakePresenter{}
	itr := NewInteractor(&FakeRepo{ErrOnFindCollection: true, Err: e.ErrInternal}, &i.FakeArtefactRepo{}, presenter)
	itr.Execute(Request{})
...
func TestHandleArtefactRepoError(t *testing.T) {
	presenter := &FakePresenter{}
	repo := &FakeRepo{}
	itr := NewInteractor(repo, &i.FakeArtefactRepo{Err: e.ErrInternal}, presenter)
